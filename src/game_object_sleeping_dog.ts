class GameObjectSleepingDog extends GameObject {
    ...
    constructor(x: number, y: number) {
        super(x, y, GFX_DOG_SLEEPING_V2_1, 80, 120, 300, 0, GameObjectInteractionType.OverlapNonBlocking, InteractionParam.SleepingDogBowl)

        this.overlaySprites = [
            new GfxSprite(GFX_DOG_SLEEPING_OVERLAY1_V1_1),
            new GfxSprite(GFX_DOG_SLEEPING_OVERLAY2_V1_1),
            new GfxSprite(GFX_DOG_SLEEPING_OVERLAY3_V1_1),
        ]
    ...
    physicsFrame() {
        ...
        if (this.bowlTicksLeft > 280) {
            this.bowlOverlaySprites[0].moveTo(this.x + 296, this.y + 76)
        }
        else if (this.bowlTicksLeft > 140) {
            this.bowlOverlaySprites[1].moveTo(this.x + 296, this.y + 76)
        }
        else if (this.bowlTicksLeft > 0) {
            this.bowlOverlaySprites[2].moveTo(this.x + 296, this.y + 76)
        }
